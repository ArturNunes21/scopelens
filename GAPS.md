...
| G15 | `maxDuration` do Vercel free tier pode não cobrir as 3 chamadas encadeadas | Pode invalidar a decisão "sem queue" no Phase 5 | `maxDuration = 60` explícito nos 2 pontos de entrada das Server Actions (`meetings/new/layout.tsx`, `meetings/[id]/page.tsx`) | Doc+Code | ✅ Resolvido (verificação manual do plano real no dashboard Vercel ainda recomendada — ver TODO.md) |
