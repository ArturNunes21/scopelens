...
- **Billing (implemented Phase 7):** Stripe Checkout (test mode, `src/app/billing/actions.ts`) → webhook (`src/app/api/stripe/webhook/route.ts`) → updates `workspaces.plan`/`stripe_customer_id`/`stripe_subscription_id`. A feature gate (`src/lib/billing.ts`) — a meetings/month limit on the free plan — is checked in the `createMeeting` server action before accepting a new `insert` into `meetings`, reflecting the `plan` field. **Webhook security (resolves GAPS.md G20):** the handler verifies the Stripe signature (`stripe.webhooks.constructEvent`) and dedupes by `event.id` — a row is inserted into `stripe_webhook_events` (id = `event.id`, RLS-enabled with no policy, service-role-only) before the event is processed; a unique-violation on that insert means the event was already applied, so it's acknowledged without reprocessing.
